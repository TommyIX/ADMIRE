...
# DSBN/MSKT系模型专用参数---------------------------------------------------------------
lr_uni = 0.001  # 通用模型的学习率
ktweight = 1  # 一致性损失权重(MSKT)
load_ckpt_dir_uni = './checkpoints/ADMIRE_model_uni_19.pth'  # 仅用于DSBN/MSKT模型，读取通用模型目录

# 半/自监督学习参数 --------------------------------------------------------------------
load_ckpt_ema_dir = './checkpoints/ADMIRE_model_ema_1.pth'  # 仅用于MT模型，读取ema模型目录
load_ckpt_dir_encoder = './checkpoints/ADMIRE_modelenc_5.pth'  # 仅用于PSMT模型恢复训练，只需要给定student encoder路径就行，会按照格式自动读取剩下的5个
...
beta_for_Lcon = 1  # 无监督损失的权重系数
use_PSMT_T_VAT = True  # 是否使用PSMT_T_VAT
