...
class CNN_B1(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        resized_out = []
        x = self.upsample(self.pool(self.bn1(self.relu(self.conv1(x))))) # size=[bs, 64, 128, 128]
        resized_out.append(x)
        x = self.upsample(self.pool(self.bn2(self.relu(self.conv2(x)))))
        resized_out.append(x)
        x = self.upsample(self.pool(self.bn3(self.relu(self.conv3(x)))))
        resized_out.append(x)
        x = self.upsample(self.pool(self.bn4(self.relu(self.conv4(x)))))
        resized_out.append(x)
        x = self.upsample(self.pool(self.bn5(self.relu(self.conv5(x)))))
        resized_out.append(x)
